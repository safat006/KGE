...
public:
    ...
    /*
    virtual void train_triplet1( const pair<pair<int, int>,int>& triplet)
    {
        ...
        //probabilitybased_false_sampling(triplet, triplet_f);
        
        // by setting training_threshold = 0 i'm ensuring true energy is less than false energy
        // but it is generating more error
        ...
        //cout << "true: "<< prob_triplets(triplet) << " false: " << prob_triplets(triplet_f)<<" thrd: "<< training_threshold <<endl;
        //int x;
        //cin>>x;
        ...
        if (norm_L1(relation) > 1.0)
            relation = normalise(relation);
        
        if (norm_L1(head_f) > 1.0)
            head_f = normalise(head_f);
        
        if (norm_L1(tail_f) > 1.0)
	...
	virtual void train_triplet( const pair<pair<int, int>,int>& triplet)
	{
        ...
        //data_model.sample_false_triplet(triplet, triplet_f);
        //cout << "For True triplet: h:" << triplet.first.first << " r: " << triplet.second << " t: " << triplet.first.second << endl;

        energybased_false_sampling(triplet, triplet_f);
        ...
        //if (prob_triplets(triplet) - prob_triplets(triplet_f) < training_threshold)
			//return;
        // original code
        //cout << prob_triplets(triplet) - prob_triplets(triplet_f) << " < " << training_threshold << endl;
        if (prob_triplets(triplet) - prob_triplets(triplet_f) > training_threshold)
            return;
...
protected:
    //vector<vec>    embedding_weights;
    vector<mat> projection_matrix_M;
    vector<rowvec>    embedding_entity_rowvec;
    vector<rowvec>    embedding_relation_rowvec;
    //rowvec
    
public:
    TransR(const Dataset& dataset,
           const TaskType& task_type,
           const string& logging_base_path,
           int dim,
           double alpha,
    ...
    dim, alpha, training_threshold)
    {
        logging.record() << "\t[Name]\tTransR";
        logging.record() << "\t[Dimension]\t" << dim;
        logging.record() << "\t[Learning Rate]\t" << alpha;
        logging.record() << "\t[Training Threshold]\t" << training_threshold;
        
        //embedding_weights.resize(count_relation());
        projection_matrix_M.resize(count_relation());
        printf("before");
        for_each(projection_matrix_M.begin(), projection_matrix_M.end(), [&](mat& m) {
            m.resize(dim, dim);
            m = ((2 * randu(dim, dim) - 1) * sqrt(6.0 / dim));
        });
        printf("after");
        
        embedding_entity_rowvec.resize(count_entity());
        for_each(embedding_entity_rowvec.begin(), embedding_entity_rowvec.end(), [=](rowvec& elem) {elem = (2 * randu(1, dim) - 1)*sqrt(6.0 / dim); });
        
        embedding_relation_rowvec.resize(count_relation());
        for_each(embedding_relation_rowvec.begin(), embedding_relation_rowvec.end(), [=](rowvec& elem) {elem = (2 * randu(1, dim) - 1)*sqrt(6.0 / dim); });
        
        //printf("Printing projection Matrix:\n");
        //for_each(projection_matrix_M.begin(), projection_matrix_M.end(), [&](mat& m) { m.print; printf("\n NEXT \n"); });
    ...
    virtual double prob_triplets(const pair<pair<int, int>, int>& triplet)
    {
        rowvec error = (embedding_entity_rowvec[triplet.first.first] * projection_matrix_M[triplet.second]
                        + embedding_relation_rowvec[triplet.second]
                        - embedding_entity_rowvec[triplet.first.second] * projection_matrix_M[triplet.second]);
        return -sum(abs(error));
    }
    
    virtual void train_triplet(const pair<pair<int, int>, int>& triplet)
    {
        rowvec& head = embedding_entity_rowvec[triplet.first.first];
        rowvec& tail = embedding_entity_rowvec[triplet.first.second];
        rowvec& relation = embedding_relation_rowvec[triplet.second];
        mat& projection_weight = projection_matrix_M[triplet.second];
        
        rowvec val_head = head;
        rowvec val_tail = tail;
        ...
        rowvec& head_f = embedding_entity_rowvec[triplet_f.first.first];
        rowvec& tail_f = embedding_entity_rowvec[triplet_f.first.second];
        rowvec& relation_f = embedding_relation_rowvec[triplet_f.second];
        
        
        rowvec val_head_f = head_f;
        rowvec val_tail_f = tail_f;
        
        rowvec factor_true = sign(head * projection_weight + relation - tail * projection_weight);
        rowvec factor_false = sign(head_f * projection_weight + relation - tail_f * projection_weight);
        
        head -= alpha * factor_true * projection_weight.t();
        tail += alpha * factor_true * projection_weight.t();
        relation -= alpha * factor_true;
        
        head_f += alpha * factor_false * projection_weight.t();
        tail_f -= alpha * factor_false * projection_weight.t();
        relation_f += alpha * factor_false;
        
        projection_weight -= alpha * (val_head - val_tail).t() * factor_true;
        projection_weight += alpha * (val_head_f - val_tail_f).t() * factor_false;
        ...
        if (norm(head_f, 1) > 1.0)
            head_f = normalise(head_f);
        
        if (norm(tail_f, 1) > 1.0)
            tail_f = normalise(tail_f);
        
        if (norm(relation_f, 1) > 1.0)
            relation_f = normalise(relation_f);
        
        if (norm(projection_weight, 1) > 1.0)
            projection_weight = normalise(projection_weight);
    }
...
{
protected:
    vector<vec>    projected_entity;
    vector<vec>    projected_relation;
    
public:
    TransD(const Dataset& dataset,
           const TaskType& task_type,
           const string& logging_base_path,
           int dim,
           double alpha,
    ...
    dim, alpha, training_threshold)
    {
        logging.record() << "\t[Name]\tTransD";
        logging.record() << "\t[Dimension]\t" << dim;
        logging.record() << "\t[Learning Rate]\t" << alpha;
        logging.record() << "\t[Training Threshold]\t" << training_threshold;
        
        projected_entity.resize(count_entity());
        for_each(projected_entity.begin(), projected_entity.end(), [=](vec& elem) {elem = (2 * randu(dim, 1) - 1)*sqrt(6.0 / dim); });
        
        projected_relation.resize(count_relation());
        for_each(projected_relation.begin(), projected_relation.end(), [=](vec& elem) {elem = (2 * randu(dim, 1) - 1)*sqrt(6.0 / dim); });
    }
    
    virtual double prob_triplets(const pair<pair<int, int>, int>& triplet)
    {
        vec error = (projected_relation[triplet.second] * projected_entity[triplet.first.first].t() + eye(dim, dim)) * embedding_entity[triplet.first.first]
        + embedding_relation[triplet.second]
        - (projected_relation[triplet.second] * projected_entity[triplet.first.second].t() + eye(dim, dim)) * embedding_entity[triplet.first.second];
    ...
    virtual void train_triplet(const pair<pair<int, int>, int>& triplet)
    {
        ...
        vec& tail = embedding_entity[triplet.first.second];
        vec& relation = embedding_relation[triplet.second];
        vec& relation_p = projected_relation[triplet.second];
        vec& head_p = projected_entity[triplet.first.first];
        vec& tail_p = projected_entity[triplet.first.second];
        ...
        vec factor_true = sign((relation_p * head_p.t() + eye(dim, dim)) * head
                               + relation
                               - (relation_p * tail_p.t() + eye(dim, dim)) * tail);
        
        vec factor_false = sign((relation_p * head_p.t() + eye(dim, dim)) * head_f
                                + relation
                                - (relation_p * tail_p.t() + eye(dim, dim)) * tail_f);
        
        head -= alpha * (relation_p * head_p.t() + eye(dim, dim)) * factor_true;
        tail += alpha * (relation_p * tail_p.t() + eye(dim, dim)) * factor_true;
        relation -= alpha * factor_true;
        
        head_f += alpha * (relation_p * head_p.t() + eye(dim, dim)) * factor_false;
        tail_f -= alpha * (relation_p * tail_p.t() + eye(dim, dim)) * factor_false;
        relation_f += alpha * factor_false;
        
        head_p -= alpha * (relation_p * head.t()) * factor_true;
        head_p += alpha * (relation_p * head.t()) * factor_false;
        
        tail_p += alpha * (relation_p * tail.t()) * factor_true;
        tail_p -= alpha * (relation_p * tail.t()) * factor_false;
        
        relation_p -= alpha * ((head_p * head.t()) - (tail_p * tail.t())) * factor_true;
        relation_p += alpha* ((head_p * head.t()) - (tail_p * tail.t())) * factor_false;
        
        // need to complete for factor_false
        ...
        if (norm_L2(tail_p) > 1.0)
            tail_p = normalise(tail_p);
        
        if (norm_L2(head_p) > 1.0)
            head_p = normalise(head_p);
        
        if (norm_L2(relation_p) > 1.0)
            relation_p = normalise(relation_p);
